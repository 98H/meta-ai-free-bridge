...
def clean_guidance_cards(content: str, is_server: bool = False) -> str:
    ...
    hint_meta = f',("openai-compatible-chat-meta"==={var}||"meta"==={var})&&{jsx}("p",{{className:"text-xs text-brand-600 dark:text-brand-400 bg-brand-500/10 border border-brand-500/20 p-2.5 rounded-lg mt-1 font-sans leading-relaxed break-words",children:"💡 Where to get token: Sign in to meta.ai → F12 → Application tab → Cookies → https://www.meta.ai → copy \'ecto_1_sess\' (or copy Cookie from Network tab). Click \'Test Connection\' before saving."}})'
